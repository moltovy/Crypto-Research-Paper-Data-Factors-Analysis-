...
from cqresearch.research.data_foundation import build_data_foundation, build_data_foundation_figures
from cqresearch.research.registries import build_foundation_registries
...
from cqresearch.research.samples import build_sample_registries


def run_research(module: str = "all", root: Path = PROJECT_ROOT) -> list[Path]:
    if module == DATA_FOUNDATION_ID:
        artifacts = build_foundation_registries(root)
        artifacts.extend(build_sample_registries(root))
        artifacts.extend(build_data_foundation(root))
        return artifacts
    if module == "all":
        artifacts = build_foundation_registries(root)
        artifacts.extend(build_sample_registries(root))
...
def build_research_figures(module: str = "all", root: Path = PROJECT_ROOT) -> list[Path]:
    if module == DATA_FOUNDATION_ID:
        return build_data_foundation_figures(root)
    if module == "all":
        artifacts = [
            *build_data_foundation_figures(root),
            *build_research_only_figures("all", root),
        ]
        artifacts.extend(build_root_research_surface(root))
        return artifacts
    if module in MODULE_SPECS:
